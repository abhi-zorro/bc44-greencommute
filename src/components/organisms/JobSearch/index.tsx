...
type NewValueType = string | null | unknown;
...
const FindJobs: React.FC<CommonProps> = () => {
  //mocking user skills and job locations till server is setup
  const userSkills = ['UI/UX Designer', 'Python Developer', 'Java Developer'];
  const jobLocations = ['Hyderabad', 'Secunderabad', 'Banglore'];
  const [skills, setSkills] = useState<NewValueType>(userSkills[0]);
  const [jobs, setJobs] = useState<NewValueType>(jobLocations[0]);
  ...
  const clickHandlerWrap = (_event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    console.log('entered value: ' + skills, jobs);
  };
