...
const JobDescriptionView = () => {
  ...
  const { userLocation, id } = contextData.state as UserLocationType;

  let previousElement: any = document.getElementById(id);
  ...
  const alterOrder = (data: Job[], cardId: string) => {
    ...
    let temp = data?.map((job) => {
      if (job.id === cardId) {
        ...
        let initCardDetails = data[count];
        setDetails(initCardDetails.details);
      }
      count++;
    });
  };
